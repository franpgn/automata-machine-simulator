...
def _parse_transition_function(tf_dict):
    parsed = {}
...
def _create_machine(config):
    machine_type = config.get('machine_type', '').lower()
    if machine_type == 'turing':
        transition_function = _parse_transition_function(config.get('transition_function', {}))
...
class Simulator:
    def __init__(self, input_json_path):
        ...
        self.machine = _create_machine(self.config)
